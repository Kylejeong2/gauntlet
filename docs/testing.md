...
The durable-engine suite proves serial specialist execution through checkpoints, one synthesis after stored results, cleanup after review work, the full ten-reviewer bounded retry reservation of $0.171, end-to-end phase advancement, restart without repeating a stored reviewer report, deterministic-name Sailbox reconciliation after a lost creation receipt, and GitHub review reconciliation after a lost publication receipt. Store tests also prove that semantic duplicate findings collapse before their atomic checkpoint and that an expired publisher cannot record a receipt after a replacement worker claims the intent. Pure publication-policy tests cover distinct challenge requirements, missing-result closure, challenge-aware readiness normalization, deterministic no-findings summaries, and final publication shaping.
